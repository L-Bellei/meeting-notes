...
- **Quatro hooks irmãos em `useMeeting.ts` com o mesmo defeito de invalidação de cache** — `useGenerateSummary:97`, `useGenerateKeyPoints:105`, `useReprocess:89` e `useRetranscribe:134` invalidam só `["meeting", meetingId]`, mas alimentam telas do board que leem `summary`/`key_points`/`tasks` de `["board-card", id]`. É a mesma forma do bug corrigido no checkbox de task (PR #45) e, na branch `feat/card-detail-modal-ux`, no `useMoveCard` e no `useGenerateTasks` (ver CHANGELOG [2026-08-22]). Ficou fora do escopo dessa branch porque nenhuma das 9 tasks do plano dependia deles — alargar a correção ali traria comportamento que ninguém pediu para dentro da review. A review final da branch decide se a inconsistência (um hook do arquivo corrigido, quatro não) é resolvida antes do merge ou fica para depois.
