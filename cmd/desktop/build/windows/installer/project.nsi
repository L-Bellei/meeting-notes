...
## Signing hooks (uncomment to enable):
...
Function .onInit
    ...
    # Read the recorded install location so we can pass _?= to the uninstaller.
    # _?= prevents the uninstaller from spawning a copy in %TEMP%, which lets
    # ExecWait actually block until uninstallation completes.
...
Section
    ...
    # Store install location so the next upgrade can find and pass _?= to the uninstaller
